...
$creply=$_POST['reply'];					  
$concatreply=';'.$creply;
$query1="update step2shopify_complaints	set status=0,c_description=CONCAT(c_description,:creply),adminid='".$admin."'
			where complaintid='".$id."'";		
$result1 = oci_parse($connection, $query1);
oci_bind_by_name($result1,':creply',$concatreply);
oci_execute($result1);
						  
						  
$query = "select COMPLAINTID,c_description,COMPLAINTTIME,priority from step2shopify_complaints where status=1 and adminid is not null
group by COMPLAINTID,c_description,COMPLAINTTIME,priority order by priority,COMPLAINTTIME";
$statement = oci_parse($connection, $query);
$res=oci_execute($statement,OCI_DEFAULT);
if($res){
	$suc="Reply Sent Successfully";
}
else{
	$suc="Reply not sent... check the connection";
}
//Pagination_START
...
<input type="submit" value="Reply"></h4>
</div>
</form>
<?php echo $suc; ?>
</div>
